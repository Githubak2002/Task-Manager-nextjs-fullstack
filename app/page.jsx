...
const inputCss = "w-3/4 p-2 border-transparent rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 bg-[#EDF2F7] focus:bg-transparent"
...
export default function Home() {
  ...
  return (
    <section className="min-h-[70vh]">
      ...
      <h2 className="gradient-text2 font1 text-4xl text-center py-5 ">TODO Next js</h2>

      <form onSubmit={submitHandler} className="space-x-4 mx-auto max-w-[540px] px-3 text-sm flexCenter">
        <input
          type="text"
          // name="task"
          value={task}
      ...
      <main className="flexCenter flex-col px-3 mt-8">
        {allTodo?.length !== 0 && <h2 className="gradient-text text-lg font-bold pb-3">All your todos</h2> }
        { 
          allTodo.map((ele,i) => {
            return(
              <div key={i} className={`border-gray-300 ${allTodo.length == i-1 ? "border-none" : "border-b"} ${i === allTodo.length - 1 ? "border-none" : "border-b"} px-3 py-3 flexBetween md:w-[460px] w-full mx-auto text-xs`}>
                <h2>{ele.task} </h2>
